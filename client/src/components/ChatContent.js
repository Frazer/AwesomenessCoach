...
function ChatContent({ chatHistory, userInfoMap }) {
  ...
  for (let i = 0; i < timeGrouped.length; i++) {
    ...
    const currGroupContent = [<div className="time-marker" key={ currGroup.time }>{ formattedTime }</div>];
    ...
    for (let j = 0; j < msgGroupsBySender.length; j++) {
      currGroupContent.push(
        <ChatMessageGroup
          ...
          key={ msgGroupsBySender[j].messages[0].id + msgGroupsBySender[j].senderId }
        />
      );
    }
    contentList.push(
      <div className="time-group">{ currGroupContent }</div>
    );
  }
...
function groupByUserAndShortTime(chatMessages) {
  ...
  const timeGap = 60;
  const groups = [];
  let currGroup = { senderId: chatMessages[0].senderId, messages: [chatMessages[0]] };

  for (let i = 1; i < chatMessages.length; i++) {
    // Create new group
    if ((chatMessages[i].time - chatMessages[i - 1].time) > timeGap) {
      groups.push(currGroup);
      currGroup = { senderId: chatMessages[i].senderId, messages: [chatMessages[i]] };
    }
    // Continue current group
    else {
      currGroup.messages.push(chatMessages[i]);
